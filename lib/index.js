...
import { Esp32MultiManager } from './esp32Manager.js'
...
class UnderDeck extends EventEmitter {
    Esp32MultiManager = new Esp32MultiManager();
    Api = new _api();
    Socket = null;
        ...
        return new Promise(async (resolve) => {
            ...
            this.Socket.onopen = () => {
                ...
                if (this.Pc && this.Pc.id) {
                    this.SendSocketMessage({ Id: this.Pc.id }, 'Ready');
                }   
            };  
            this.Socket.onmessage = (event) => {
                ...
                try {
                    ...
                    objectReceived.object = obj;
                    objectReceived.reply = async (Message, Method = 'Reply') => {
                        return this.SendSocketMessage({ ...Message, to: obj.From }, Method);
                    }
                } catch (error) {
                    objectReceived.string = event.data.toString();
                    objectReceived.reply = async (Message) => {
                        return false;
                    }
                }
                ProcessFC.ProcessWebSocketMessages(this, objectReceived);
            };  
    ...
    async RegisterThisPc(){
        try {
            ...
            if(response && response.data && response.data.pc){
                ...
                if(this.Pc && this.Pc.id) await this.SendSocketMessage({ Id: this.Pc.id }, 'Ready');
                return this.Pc;
            } else {
                return null;
            }
    ...
    */
    async SendSocketMessage(Message, Method = null){
        try {
            if (this.Socket && this.Socket.readyState === WS.OPEN) {
                try {
                    let object = {
    ...
    */
    ...
    /**
    * Get List Themes for app and user.
    * @returns {Promise<Object>} A Promise that resolves to a array, `false` from the API, or `null` if an error occurs or the client is not authenticated.
    */
    async GetThemes(){
        return new Promise(async (resolve) => {
            try {
    ...
    /**
    * Get List Plugins for app.
    * @returns {Promise<Object>} A Promise that resolves to a array, `false` from the API, or `null` if an error occurs or the client is not authenticated.
    */
    async GetPlugins(){
        return new Promise(async (resolve) => {
            try {
                let response = await this.Api.Request({ Method: 'GetPlugins' });
                resolve(response.data && response.data.plugins ? response.data.plugins : []);
            } catch (error) {
                this.emit('UndError', error);
                resolve([]);
            }
    ...
    */
    ...
    /**
    * Upload data to save in cloud.
    * @param {Object} DataToUpload - Object contain data to upload.
    ...
    */
    async CloudUploadData(DataToUpload, callBack){
        return new Promise(async (resolve) => {
            try {
                if(!this.User) resolve(false);
                await this.SocketConnect();
                await ProcessFC.UploadFilesToCloud(this, DataToUpload, callBack).then(async DataFiles => {
                    try {
                        let response = await this.Api.Request({
                            Method: 'CloudSyncData',
                            ClientToken: this.User.token,
                            Json: JSON.stringify(DataToUpload.Itens),
    ...
    /**
    * Get user data saved in cloud.
    * @returns {Promise<Object>} A Promise that resolves to a object, `false` from the API, or `null` if an error occurs or the client is not authenticated.
    */
    async GetSynchronizedData(){
        return new Promise(async (resolve) => {
            try {
                let response = await this.Api.Request({
                    Method: 'GetSynchronizedData',
                    ClientToken: this.User.token,
                });
                resolve(response.data);
            } catch (error) {
    ...
    /**
    * Clear user data saved in cloud.
    * @returns {Promise<Object>} A Promise that resolves to a object, `false` from the API, or `null` if an error occurs or the client is not authenticated.
    */
    async ClearSynchronizedData(){
        return new Promise(async (resolve) => {
            try {
                let response = await this.Api.Request({
                    Method: 'ClearSynchronizedData',
                    ClientToken: this.User.token,
                });
                resolve(response.data);
            } catch (error) {
    ...
    /**
    * Get List Themes for app and user.
    * @returns {Promise<Object | boolean>} A Promise that resolves to a User object, `false` from the API, or `false` if an error occurs or the client is not authenticated.
    */
    ...
    /**
    * Get List Themes for app and user.
    * @param {string} Username - Inform username or
    * @param {string} Email - User account email.
    * @returns {Promise<Object | boolean>} A Promise that resolves to object, `false` from the API, or `false` if an error.
    */
    async RequestCodeChangePassword(Username = String()){
        return new Promise(async (resolve) => {
            try {
                let response = await this.Api.Request({ Method: 'RequestCodeChangePassword', Username: Username, });
                resolve(response.data);
            } catch (error) {
                this.emit('UndError', error);
                resolve(false);
    ...
    /**
    * Get List Themes for app and user.
    * @param {String} ClientId - Enter the customer ID received with the code request to change the password.
    * @param {String} Code - Code to confirm password change.
    * @param {String} Password - New Password.
    * @param {String} CPassword - Confirmation password.
    * @returns {Promise<Object | boolean>} A Promise that resolves to object, `false` from the API, or `false` if an error.
    */
    async ChangePassword(ClientId = String(), Code = String(), Password = String(), CPassword = String()){
        return new Promise(async (resolve) => {
            try {
                let response = await this.Api.Request({ Method: 'ChangePassword', ClientId: ClientId, Code: Code, Password: Password, CPassword});
                resolve(response.data);
            } catch (error) {
                this.emit('UndError', error);
                resolve(false);
    ...
    */
    async Logout(){
        return new Promise(async (resolve) => {
            try {
                await this.SendSocketMessage({ ClientId: this.User.id }, 'Logout');
                this.Pc = null;
                this.User = null;
                this.emit('Logout');
                resolve(true);
