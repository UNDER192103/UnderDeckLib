    ...
    */
    ...
    constructor() {
        ...
        this.on('Ready', async () => {
            ...
            if(this.User && this.User.id) await this.SendSocketMessage({ Method: 'Ready', ClientId: this.User.id });
        });
    ...
    async SocketConnect(){
        return new Promise(async (resolve) => {
            ...
            this.Socket.on('message', (data) => {
                try {
                    this.emit('SocketMessage', { string: data.toString(), object: JSON.parse(data.toString()) });
                } catch (error) {
                    this.emit('SocketMessage', { string: data.toString(), object: null });
                }
            });
